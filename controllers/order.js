...
const createOrder = async (req, res) => {
  ...
  const { items, status, date, deliveryInfo, paymentMethod } = req.body;
...
if (paymentMethod === "card") {
  ...
  const paymobOrder = await registerOrder(token, newOrder._id, totalPrice * 100, items);
  ...
  const paymentToken = await generatePaymentKey(
    ...
    paymobOrder.id,
    {
      ...
      email: deliveryInfo.email || "test@example.com",
  ...
  //  iframeUrl
  const url = iframeUrl(paymentToken);

  return res.status(201).json({
    message: "Order created, continue payment with Paymob",
    order: newOrder,
    iframeUrl: url,
  });
}
